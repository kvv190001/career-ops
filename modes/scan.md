...
**Every company MUST have a `careers_url` in portals.yml.** If missing, look it up once, save it, and use it for future scans.
...
- `workday`: `jobPostings[]`/`jobPostings` (depends on tenant) → `title`, `externalPath` or URL built from the host
...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   **Do not abort the entire scan if a URL fails.** If `browser_navigate` errors (timeout, 403, etc.), mark as `skipped_expired` and continue with the next one.
...
Every company in `tracked_companies` must have a `careers_url` — the direct URL to its jobs page. This avoids looking it up every time.
...
Fallback: if you only have the direct ATS URL, first navigate to the company's website and locate their corporate careers page. Use the direct ATS URL only if the company has no corporate careers page.
